...
class TestCompletionLogic(unittest.TestCase):
    ...
    def _make_app(self, comp_items=None, comp_index=-1):
        ...
        from EvoScientist.cli.tui_interactive import _TUI_SLASH_COMMANDS
        ...
        class _StubApp:
            ...
            def __init__(self):
                ...
                self._SLASH_COMMANDS = _TUI_SLASH_COMMANDS
            ...
            def on_key(self, key: str):
                ...
                if key == "down":
                    ...
                    self._apply_selected_completion()
                    return True
                elif key == "enter" and self._comp_index >= 0:
                    self._hide_completions()
                    return True
                return False
    ...
    # ------------------------------------------------------------------
    # on_key  (down / up / enter)
    # ------------------------------------------------------------------

    def test_on_key_down_cycles_forward(self):
        items = [("/resume", "d1"), ("/run", "d2"), ("/reset", "d3")]
        app = self._make_app(comp_items=items, comp_index=0)
        app._fake_completions.display = True
    ...
    def test_on_key_up_cycles_backward(self):
        items = [("/resume", "d1"), ("/run", "d2")]
        app = self._make_app(comp_items=items, comp_index=1)
        app._fake_completions.display = True

        handled = app.on_key("up")
        assert handled is True
        assert app._comp_index == 0

    def test_on_key_up_wraps_to_last(self):
        items = [("/resume", "d1"), ("/run", "d2")]
        app = self._make_app(comp_items=items, comp_index=0)
        app._fake_completions.display = True

        app.on_key("up")
        assert app._comp_index == 1  # wrapped to last
